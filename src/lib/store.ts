...
export interface User {
  ...
  licenseNumber?: string;
    ...
    trainerStatus?: 'none' | 'pending' | 'approved' | 'rejected';
    createdAt: string;
  }
...
export interface Course {
  ...
  replays?: { title: string; url: string; date: string }[];
  price: number;
  isFree: boolean;
  zoomUrl?: string;
}
...
export interface TrainingEnrollment {
  ...
  quizResults?: { moduleId: string; score: number }[];
  grade?: string;
  certificateUrl?: string;
  createdAt: string;
}
...
export interface AppState {
  ...
  addAvailability: (availability: Omit<DoctorAvailability, 'id' | 'createdAt'>) => Promise<void>;
  updateAvailability: (id: string, data: Partial<DoctorAvailability>) => Promise<void>;
  ...
  addSubmission: (submission: Omit<AssessmentSubmission, 'id' | 'submittedAt'>) => Promise<void>;
  updateSubmission: (id: string, data: Partial<AssessmentSubmission>) => Promise<void>;

  diplomas: Diploma[];
  addDiploma: (diploma: Omit<Diploma, 'id' | 'issuedAt' | 'emailSent'>) => Promise<void>;
  sendDiplomaEmail: (id: string) => Promise<void>;
  ...
  requestToJoinSchool: (schoolId: string) => Promise<void>;
  respondToTrainerRequest: (trainerId: string, status: 'approved' | 'rejected') => Promise<void>;
    ...
    (set, get) => ({
      ...
      addUser: (user) => set((state) => ({ users: [...state.users, user] })),
            updateUser: async (id, data) => {
              ...
              if (data.trainerStatus) updateData.trainer_status = data.trainerStatus;
            ...
            requestToJoinSchool: async (schoolId) => {
              const { user, updateUser } = get();
              if (!user) return;
              await updateUser(user.id, { schoolId, trainerStatus: 'pending' });
              const school = get().schools.find(s => s.id === schoolId);
              if (school) {
                // In a real app, notify the school owner
                get().addNotification({
                  message: `Demande envoyée à ${school.name}. En attente de confirmation.`,
                  type: 'info'
                });
              }
            },

            respondToTrainerRequest: async (trainerId, status) => {
              const { updateUser } = get();
              const trainerStatus = status === 'approved' ? 'approved' : 'rejected';
              await updateUser(trainerId, { trainerStatus });
              get().addNotification({
                message: `Le formateur a été ${status === 'approved' ? 'approuvé' : 'refusé'}.`,
      ...
      updateAvailability: async (id, data) => {
        ...
        if (data.slotDuration) updateData.slot_duration = data.slotDuration;
        const { error } = await supabase.from('doctor_availability').update(updateData).eq('id', id);
        if (!error) set((state) => ({
          doctorAvailability: state.doctorAvailability.map(a => a.id === id ? { ...a, ...data } : a)
        }));
      },
      deleteAvailability: async (id) => {
        ...
        if (!error) set((state) => ({
          doctorAvailability: state.doctorAvailability.filter(a => a.id !== id)
        }));
      },
      ...
      updateEnrollment: async (id, data) => {
        ...
        if (data.quizResults) { updateData.quiz_results = data.quizResults; delete updateData.quizResults; }
        ...
        pharmacies: [],
          ...
          updateSchool: async (id, data) => {
            ...
            if (data.phone) updateData.phone = data.phone;
            if (data.email) updateData.email = data.email;
            if (data.ccp !== undefined) updateData.ccp = data.ccp;
            if (data.baridimob !== undefined) updateData.baridimob = data.baridimob;
      ...
      userProgress: {},
        ...
        addCourse: async (course) => {
          ...
          const { error } = await supabase.from('courses').insert({
            ...
            recorded_sessions: course.replays,
            schedule: course.schedule,
            content_plan: course.content_plan,
        ...
        updateCourse: async (id, data) => {
          ...
          if (data.schoolId) { updateData.school_id = data.schoolId; delete updateData.schoolId; }
          ...
          if (data.replays) { updateData.recorded_sessions = data.replays; delete updateData.replays; }
          if (data.contentPlan) { updateData.content_plan = data.contentPlan; delete updateData.contentPlan; }
        ...
        addAssessment: async (assessment) => {
          ...
          const { data, error } = await supabase.from('assessments').insert({
            ...
            is_secured: assessment.isSecured
          }).select().single();
          if (!error && data) set((state) => ({
        ...
        updateAssessment: async (id, data) => {
          const supabase = createClient();
          const updateData: any = { ...data };
          if (data.courseId) { updateData.course_id = data.courseId; delete updateData.courseId; }
          if (data.isSecured !== undefined) { updateData.is_secured = data.isSecured; delete updateData.isSecured; }
        ...
        addDiploma: async (diploma) => {
          const supabase = createClient();
          const { data, error } = await supabase.from('diplomas').insert({
            course_id: diploma.courseId,
            student_id: diploma.studentId,
            certificate_url: diploma.certificateUrl
          }).select().single();
          if (!error && data) set((state) => ({
        ...
        sendDiplomaEmail: async (id) => {
          const diploma = get().diplomas.find(d => d.id === id);
          if (!diploma) return;
          const student = get().users.find(u => u.id === diploma.studentId);
          if (!student) return;
          ...
          try {
            const response = await fetch('/api/contact', {
              ...
              body: JSON.stringify({
                to: student.email,
                subject: "Félicitations ! Votre diplôme est prêt",
                text: `Bonjour ${student.fullName}, vous avez réussi votre formation. Voici le lien vers votre diplôme : ${diploma.certificateUrl}`
              })
            });
            if (response.ok) {
              const supabase = createClient();
              await supabase.from('diplomas').update({ email_sent: true }).eq('id', id);
              set((state) => ({
                diplomas: state.diplomas.map(d => d.id === id ? { ...d, emailSent: true } : d)
              }));
              get().addNotification({ message: "Diplôme envoyé par mail !", type: 'success' });
            }
          } catch (err) {
            get().addNotification({ message: "Erreur lors de l'envoi du mail.", type: 'error' });
          }
        },
      ...
      addNotification: (notification) => set((state) => ({
        ...
        deleteAppointment: async (id) => {
          ...
          fetchInitialData: async () => {
            ...
            try {
              const [
                { data: pharmacies, error: pErr },
                { data: factories, error: fErr },
                { data: schools, error: sErr },
                { data: medicines, error: mErr },
                { data: orders, error: oErr },
                { data: courses, error: cErr },
                { data: jobs, error: jErr },
                { data: prescriptions, error: prErr },
                { data: production, error: pdErr },
                { data: factoryInv, error: fiErr },
                { data: factoryOrd, error: foErr },
                { data: cnas, error: cnErr },
                { data: users, error: uErr },
                { data: appts, error: aErr },
                { data: enrls, error: eErr },
                { data: dossiers, error: dErr },
                { data: asms, error: asErr },
                { data: subs, error: subErr },
                { data: dips, error: dipErr },
                { data: availabilities, error: avErr }
              ] = await Promise.all([
              ...
              if (pErr) console.error('Error fetching pharmacies:', pErr);
              ...
              if (users) set({ users: users.map((u: any) => ({ 
                ...
                trainerStatus: u.trainer_status
              })) });
              ...
              if (courses) set({ courses: courses.map((c: any) => ({ ...c, trainerId: c.trainer_id, schoolId: c.school_id, status: c.status, titleAr: c.title_ar || c.title, descriptionAr: c.description_ar || c.description, modules: c.modules || [], materialsUrls: c.materials_urls || [], zoomUrl: c.zoom_url, replays: c.recorded_sessions || [], schedule: c.schedule, content_plan: c.content_plan, homeworks: c.homeworks })) });
              ...
              if (enrls) set({ enrollments: enrls.map((e: any) => ({ ...e, courseId: e.course_id, userId: e.user_id, progress: e.progress || 0, quizResults: e.quiz_results || [], createdAt: e.created_at })) });
              if (asms) set({ assessments: asms.map((a: any) => ({ ...a, courseId: a.course_id, isSecured: a.is_secured, createdAt: a.created_at })) });
              if (subs) set({ submissions: subs.map((s: any) => ({ ...s, assessmentId: s.assessment_id, studentId: s.student_id, submittedAt: s.submitted_at })) });
              if (dips) set({ diplomas: dips.map((d: any) => ({ ...d, courseId: d.course_id, studentId: d.student_id, issuedAt: d.issued_at, emailSent: d.email_sent, certificateUrl: d.certificate_url })) });
              if (availabilities) set({ doctorAvailability: availabilities.map((a: any) => ({ ...a, doctorId: a.doctor_id, dayOfWeek: a.day_of_week, startTime: a.start_time, endTime: a.end_time, slotDuration: a.slot_duration, createdAt: a.created_at })) });
